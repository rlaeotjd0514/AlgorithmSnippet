// dijkstra_minimum_Value.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
int main()
{
	...
	while (arr.size() > 0) {
		...
		arr.pop();
		if (cost[current] != MAXVAL) continue;
		cost[current] = val;
		if (current == e) break;
		for (auto it : bus[current]) {
			if (cost[it.first] != MAXVAL) continue;
			arr.push({ it.second + val, it.first });
		}
	}
